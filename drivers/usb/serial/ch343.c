...
/*
 * minor/index 到 struct ch343 的映射表。
 *
 * ttyCH343USB0、ttyCH343USB1 这些节点的数字部分最终会变成 tty->index。
 * install/open 时，驱动通过这个 index 找回 probe 阶段创建的 struct ch343，
 * 从而知道这个节点对应哪一个 USB 设备、哪一组端点和哪一个串口通道。
 */
			     ...
			     u16 index)
{
	...
	/* control OUT: 主机向芯片发送 vendor request，常用于改配置。 */
			    ...
			    u16 index, char *buf, unsigned bufsize)
{
	...
	/* control IN: 主机从芯片读取 vendor 数据，例如芯片类型/版本。 */
	retval = usb_autopm_get_interface(ch343->control);
	if (retval)
		return retval;
	retval = usb_control_msg(
				 ...
				 void *buf, unsigned bufsize)
{
	...
	/*
	 * 给 ch343_iodev%d 这类厂商辅助节点使用：用户态传入完整的
	 * request/type/value/index/buffer，驱动代发 USB control OUT。
	 */
				...
				void *buf, unsigned bufsize)
{
	...
	/* ch343_iodev%d 的通用 control IN 入口，读取结果再 copy_to_user。 */
	buffer = kmalloc(bufsize, GFP_KERNEL);
	if (!buffer)
		return -ENOMEM;
...
{
	/*
	 * 设置 DTR/RTS/自动流控等输出控制线。
	 * 多串口芯片不同 iface 使用不同 request 编号，所以这里按 iface
	 * 把 CMD_C2 映射到具体 UART 通道。
	 */
	if (ch343->iface <= 1)
...
static int ch343_configure(struct ch343 *ch343)
{
	...
	/*
	 * CMD_C6 读取芯片版本/类型。后面的 switch 会据此判断具体芯片、
	 * 串口数量和是否支持额外 IO 功能。
	 */
...
static int ch343_wb_alloc(struct ch343 *ch343)
{
	...
	/*
	 * 从 CH343_NW 个写缓冲中挑一个空闲项。写路径是异步的：
	 * tty write() 返回时，USB 传输可能还没完成，所以要用 use 标记占用。
	 */
	wbn = 0;
	i = 0;
	for (;;) {
...
{
	/* bulk OUT 完成后释放写缓冲，并归还运行时 PM 引用。 */
	wb->use = 0;
	ch343->transmitting--;
...
static int ch343_start_wb(struct ch343 *ch343, struct ch343_wb *wb)
{
	...
	/* 把已经填好数据的写缓冲挂到 bulk OUT URB 上并提交给 USB core。 */
	ch343->transmitting++;
				...
				size_t len)
{
	...
	/*
	 * interrupt IN 状态包的格式会随芯片系列略有差异。这里先取出
	 * type 字段，再按 bit 判断是 modem 状态、break、溢出还是校验错误。
	 */
	...
	if (type & CH343_CTT_M) {
		...
		/*
		 * CLOCAL 没设置时，DCD 表示“外部连接是否仍在”。如果 DCD 掉线，
		 * tty 层需要收到 hangup，用户态 read/write 会表现为串口断开。
		 */
		if (!ch343->clocal &&
		...
		if (difference) {
			/* 记录 CTS/DSR/RI/DCD 的变化次数，供 TIOCGICOUNT/TIOCMIWAIT 使用。 */
...
static void ch343_ctrl_irq(struct urb *urb)
{
	...
	/*
	 * interrupt URB 回调：处理一次状态包后马上重新提交同一个 URB。
	 * 这样驱动才能持续监听 CTS/DSR/DCD/RI 和错误状态变化。
	 */
	switch (status) {
	case 0:
		/* success */
				 ...
				 gfp_t mem_flags)
{
	...
	/* 提交一个 bulk IN URB，设备后续收到 UART 字节时会回调回来。 */
...
static void ch343_process_read_urb(struct ch343 *ch343, struct urb *urb)
{
	...
	/* 把 USB bulk IN 收到的原始字节交给 tty flip buffer，再由行规程送到用户态。 */
...
static void ch343_read_bulk_callback(struct urb *urb)
{
	...
	/*
	 * bulk IN 回调：处理本次收到的数据，然后把同一个读 URB 再挂回去。
	 * 这就是串口接收方向持续不断工作的原因。
	 */
	if (!ch343->dev) {
...
static void ch343_write_bulk(struct urb *urb)
{
	...
	/*
	 * bulk OUT 完成回调：USB core 告诉驱动这批数据已经发完或出错。
	 * 释放写缓冲后唤醒 tty 层，用户态/行规程就可以继续写下一批数据。
	 */
			     ...
			     struct tty_struct *tty)
{
	...
	/*
	 * tty 节点第一次被安装时，根据 tty->index 找回 probe 阶段保存的
	 * struct ch343，并挂到 tty->driver_data。后续 open/write/ioctl 都靠它。
	 */
...
static int ch343_tty_open(struct tty_struct *tty, struct file *filp)
{
	...
	/* open 时先把当前 termios 下发给芯片，再交给 tty_port_open 激活端口。 */
	if (tty)
...
#endif
{
	...
	/* tty 层拉高/拉低 DTR、RTS 时，转换为芯片的 CMD_C2 控制命令。 */
#ifdef IGNORE_RTSDTR
	return;
#endif
			       ...
			       struct tty_struct *tty)
{
	...
	/*
	 * 串口真正开始工作的入口。这里提交 interrupt 状态 URB 和 bulk IN
	 * 读 URB；从这一刻起，设备发来的状态和数据才会持续回调到驱动。
	 */
	mutex_lock(&ch343->mutex);
	if (ch343->disconnected)
		goto disconnected;
...
#endif
{
	...
	/*
	 * tty write() 的主路径：申请写缓冲 -> 复制用户字节 -> 提交 bulk OUT。
	 * 注意这里最多发送 writesize 字节，剩余数据由 tty 层后续再次调用 write。
	 */
	if (!count)
		return 0;
...
retry:
	...
	if (ch343->susp_count) {
		/* 设备处于 suspend 时先 anchor，resume 后再真正提交。 */
			   ...
			   unsigned long arg)
{
	...
	/*
	 * tty ioctl 入口。标准 ioctl 处理串口计数/等待线状态变化；
	 * IOCTL_CMD_* 是厂家扩展，允许用户态直接发 control transfer。
	 */
		     ...
		     unsigned char *dvs)
{
	...
	/*
	 * 根据目标波特率 bval 和具体芯片类型，计算 CH343 私有寄存器需要的
	 * fct/dvs 参数。termios 里的“115200”不会原样下发硬件，而会先走
	 * 这里换算成芯片能识别的分频值。
	 */
	buffer = kmalloc(size, GFP_KERNEL);
	if (!buffer)
...
#endif
{
	...
	/*
	 * Linux 的 termios 是通用串口配置；这里把它翻译成 CH343 的私有
	 * control request：
	 *   - 波特率 -> ch343_get() 算 fct/dvs
	 *   - 数据位/校验/停止位 -> reg_value
	 *   - DTR/RTS/硬件流控 -> ch343_set_control()
	 */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 7, 0))
	if (termios_old &&
	...
	if (C_BAUD(tty) == B0) {
		/* B0 在 tty 语义里表示挂断：保持原波特率，同时拉低 DTR/RTS。 */
	...
	/* value/index 是 CMD_C1 的两个 16 bit 参数，组合了线格式和波特率分频值。 */
	value |= reg_count;
	value |= (unsigned short)reg_value << 8;

	index |= 0x00 | dvs;
	index |= (unsigned short)fct << 8;
	/* CMD_C1 真正把波特率、数据位、校验位、停止位写进芯片。 */
	if (ch343->iface <= 1)
	...
	if (C_CRTSCTS(tty)) {
		/* CRTSCTS 表示启用硬件流控：让芯片参与 RTS/CTS 控制。 */
		       ...
		       const struct usb_device_id *id)
{
	...
	/*
	 * probe 是 USB core 匹配 VID/PID 后调用的入口。这里要做四件事：
	 *   1. 找到 control/data interface 和 interrupt/bulk 端点；
	 *   2. 创建 struct ch343 并分配 URB/DMA 缓冲；
	 *   3. 读取芯片类型，完成私有初始化；
	 *   4. 注册 /dev/ttyCH343USBX 和可选的 ch343_iodevX。
	 */
	...
	/*
	 * 有些芯片的描述符外观看起来像 CDC：control interface 里带
	 * Union Descriptor，指出 master/control 和 slave/data 两个接口。
	 * 注意后续控制命令仍是 vendor request，不是标准 CDC-ACM 请求。
	 */
	if ((!buffer) || (buflen == 0)) {
...
found_dev:
	...
	/*
	 * ctrl_buffer 给 interrupt IN 状态 URB 使用；read/write buffer 给
	 * bulk IN/OUT 数据面使用。这里仅分配和填充 URB，真正提交发生在 open。
	 */
	...
	/* interrupt IN URB 用于持续接收 CTS/DSR/DCD/RI 和错误状态。 */
	...
	if (ch343->iosupport && (ch343->iface == 0) &&
	    (ch343->io_intf == NULL)) {
		/*
		 * 额外的厂家辅助节点，不是主串口数据通道。用户态可通过它发
		 * 自定义 ioctl/control request。
		 */
	...
	/*
	 * 注册 tty 设备后，udev/devtmpfs 才会创建 /dev/ttyCH343USBX。
	 * 这个节点的 index/minor 之后会映射回当前 struct ch343。
	 */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 7, 0))
...
static void stop_data_traffic(struct ch343 *ch343)
{
	...
	/* 统一停止数据流：拔出、关闭、挂起都会走到这里清理正在飞行的 URB。 */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0))
...
static void ch343_disconnect(struct usb_interface *intf)
{
	...
	/*
	 * USB 设备拔出/接口释放时调用。先阻止新 I/O，再 hangup tty，
	 * 最后注销节点、停止 URB、释放缓冲和接口引用。
	 */
...
static int ch343_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	/* 自动挂起前如果仍在发送，拒绝 autosuspend，避免丢正在飞行的 bulk OUT。 */
...
static int __init ch343_init(void)
{
	...
	/*
	 * 模块加载入口：先注册 tty driver，定义节点名前缀 ttyCH343USB；
	 * 再注册 usb_driver，之后 USB core 才会用 ch343_ids 匹配设备并调用 probe。
	 */
...
#endif
	/*
	 * 默认串口参数：115200 8N1，允许接收，关闭回显/行模式/输出加工。
	 * 用户态 stty 或应用修改 termios 后，会进入 ch343_tty_set_termios。
	 */
