...
{
    return snd_soc_add_component_controls(component, &tas5825m_vol_control, 1);
}

static const struct snd_soc_component_driver soc_component_tas5825m = {
...
static int tas5825m_mute(struct snd_soc_dai *dai, int mute, int direction)
{
    ...
    u8 reg03_value = mute ? 0x0b : 0x03;
    u8 reg35_value = mute ? 0x00 : 0x11;
...
static int tas5825m_probe(struct device *dev, struct regmap *regmap)
{
    ...
    ret = regmap_register_patch(regmap, tas5825m_init_sequence,
                               ARRAY_SIZE(tas5825m_init_sequence));
    if (ret) {
        dev_err(dev, "Failed to initialize TAS5825M: %d\n", ret);
        return ret; // 替换dev_err_probe为传统错误处理
    }
    
    return devm_snd_soc_register_component(dev, &soc_component_tas5825m,
                                          &tas5825m_dai, 1);
}
